...
async def read(db, params: dict):
    try:
        logger.info(
            f"purchase.py 의 read()에 작업 요청이 들어왔습니다. --> params:{params}"
        )
        async with db.begin():  ### 트랜젝션
            if params["from"] == "update_report_json_to_db()":
                ...
                query = select(PurchaseModel , GeneratingSyntheticTasksModel)\
                    .join(GeneratingSyntheticTasksModel ,
                            GeneratingSyntheticTasksModel.premium_purchase_id == PurchaseModel.id )\
                    .where(GeneratingSyntheticTasksModel.id == synthetic_task_row_id)
                
                query_result = await db.execute(query)
                purchase_row , synthetic_task_row = query_result.one()
                return purchase_row.status_id , purchase_row.id
